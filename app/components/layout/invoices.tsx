...
export type InvoicePage =
  | "invoice-overview" | "invoicing" | "invoices" | "create-invoice"
  | "invoice-customers" | "invoice-customers-new" | "invoice-customer-details"
  | "invoice-payments" | "invoice-products" | "invoice-products-new" | "invoice-product-details"
  | "invoice-credit-notes" | "invoice-recurring" | "invoice-reminders" | "invoice-templates"
  | "invoice-tax-rates" | "invoice-settings";

interface InvoicesProps { activePage: InvoicePage | string; }
...
export default function Invoices({ activePage }: InvoicesProps) {
  ...
  const loadInvoices = useCallback(async () => {
    try {
      ...
      if(!a.ok) throw new Error(ad.error || "Failed to load invoices.");
      if(!b.ok) throw new Error(bd.error || "Failed to load invoice statistics.");
      setInvoices(Array.isArray(ad) ? ad : ad.invoices || []);
      setStats(bd.stats || bd || null);
  ...
  const openInvoice = useCallback(async (id:string) => {
    try {
      ...
      const r=await fetch(`/api/invoices/${id}`,{credentials:"include"}); const d=await r.json();
      if(!r.ok) throw new Error(d.error || "Failed to load invoice.");
      setSelected(d.invoice || d);
    } catch(e) { setError(e instanceof Error ? e.message : "Failed to load invoice."); }
    finally { setDetailLoading(false); }
  },[]);

  if(selected || detailLoading) return <InvoiceDetailsView invoice={selected} loading={detailLoading} onBack={()=>setSelected(null)} onRefresh={async()=>{if(selected) await openInvoice(selected.id);await loadInvoices();}}/>;

  if(activePage === "invoice-customers" || activePage === "invoice-customers-new" || activePage === "invoice-customer-details") return <CustomerManager initialCreate={activePage === "invoice-customers-new"}/>;
  if(activePage === "invoice-payments") return <PaymentsManager onOpenInvoice={openInvoice}/>;
  if(activePage === "invoice-products" || activePage === "invoice-products-new" || activePage === "invoice-product-details") return <ProductsManager initialCreate={activePage === "invoice-products-new"}/>;
  if(activePage === "invoice-credit-notes") return <CreditNotesManager/>;
  if(activePage === "invoice-recurring") return <RecurringInvoicesManager/>;
  if(activePage === "invoice-reminders") return <InvoiceRemindersManager/>;
  if(activePage === "invoice-templates") return <InvoiceTemplatesManager/>;
  if(activePage === "invoice-tax-rates") return <TaxRatesManager/>;
  if(activePage === "invoice-settings") return <InvoiceSettingsManager/>;
  if(activePage === "create-invoice") return <CreateInvoicePage onCreated={async()=>{await loadInvoices();}}/>;
  if(activePage === "invoices") return <AllInvoicesView onOpenInvoice={openInvoice}/>;

  return <InvoiceOverview invoices={invoices} stats={stats} loading={loading} onOpenInvoice={openInvoice} onRefresh={loadInvoices}/>;
}

function InvoicingSectionShell({ children }: { children: ReactNode }) {
  return <div className="space-y-6 text-gray-900 dark:text-gray-100">{children}</div>;
}
...
function CustomerManager({ initialCreate = false }: { initialCreate?: boolean }) {
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => { if (initialCreate && !loading) setShowForm(true); }, [initialCreate, loading]);
...
function ProductsManager({ initialCreate = false }: { initialCreate?: boolean }) {
  const [products, setProducts] = useState<any[]>([]);
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
...
function InvoiceOverview({invoices,stats,loading,onOpenInvoice,onRefresh}:{invoices:Invoice[];stats:Stats|null;loading:boolean;onOpenInvoice:(id:string)=>Promise<void>;onRefresh:()=>Promise<void>}){
 const total=stats?.total_invoiced??invoices.reduce((s,i)=>s+Number(i.total_amount||0),0), paid=stats?.total_collected??invoices.reduce((s,i)=>s+Number(i.amount_paid||0),0), due=stats?.total_outstanding??invoices.reduce((s,i)=>s+Number(i.amount_due||0),0);
 return <div className="space-y-6"><header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm text-gray-500">Sales & billing</p><h1 className="mt-1 text-2xl font-bold">Invoicing Overview</h1><p className="mt-1 text-sm text-gray-500">Create invoices, track payments and manage receivables.</p></div><a href="/invoices/new" className="rounded-xl bg-gray-900 px-4 py-2.5 text-sm font-semibold text-white dark:bg-white dark:text-gray-900"><Plus size={16} className="mr-2 inline"/>New invoice</a></header><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Total invoices" value={String(stats?.total_invoices??invoices.length)} icon={FileText}/><StatCard label="Total invoiced" value={money(total)} icon={Receipt}/><StatCard label="Collected" value={money(paid)} icon={Wallet}/><StatCard label="Outstanding" value={money(due)} icon={CircleDollarSign}/></div><section className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900"><div className="flex items-center justify-between border-b p-5 dark:border-gray-800"><div><h2 className="font-semibold">Recent invoices</h2><p className="text-xs text-gray-500">Latest billing activity</p></div><button onClick={onRefresh} className="rounded-xl border px-3 py-2 text-sm">Refresh</button></div>{loading?<div className="p-12 text-center"><Loader2 className="mx-auto animate-spin"/></div>:invoices.length===0?<div className="p-12 text-center text-sm text-gray-500">No invoices yet.</div>:<div className="divide-y dark:divide-gray-800">{invoices.slice(0,8).map(i=><button key={i.id} onClick={()=>onOpenInvoice(i.id)} className="flex w-full items-center gap-4 p-4 text-left hover:bg-gray-50 dark:hover:bg-gray-800"><Receipt size={18}/><span className="flex-1"><b>{i.invoice_number}</b><span className="ml-3 text-sm text-gray-500">{i.customer?.company_name||"Unknown customer"}</span></span><span className="font-semibold">{money(i.total_amount)}</span><span className={`rounded-full px-2.5 py-1 text-xs ${statusClass(getDisplayStatus(i))}`}>{LABELS[getDisplayStatus(i)]}</span><ChevronRight size={16}/></button>)}</div>}</section></div>;
}

type ResourceConfig={title:string;description:string;endpoint:string;icon:LucideIcon;fields:{key:string;label:string;type?:string}[]};
function ResourceManager({config}:{config:ResourceConfig}){
 const [rows,setRows]=useState<any[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState(""),[open,setOpen]=useState(false),[saving,setSaving]=useState(false),[form,setForm]=useState<Record<string,string>>({});
 const load=useCallback(async()=>{try{setLoading(true);const r=await fetch(config.endpoint,{credentials:"include"}),d=await r.json();if(!r.ok)throw new Error(d.error||"Failed to load records.");const list=Array.isArray(d)?d:(d.items||d.data||Object.values(d).find(v=>Array.isArray(v))||[]);setRows(Array.isArray(list)?list:[]);}catch(e){setError(e instanceof Error?e.message:"Failed to load records.");}finally{setLoading(false);}},[config.endpoint]);
 useEffect(()=>{load();},[load]);
 const save=async()=>{try{setSaving(true);const r=await fetch(config.endpoint,{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(form)}),d=await r.json();if(!r.ok)throw new Error(d.error||"Failed to save record.");setOpen(false);setForm({});await load();}catch(e){setError(e instanceof Error?e.message:"Failed to save record.");}finally{setSaving(false);}};
 const Icon=config.icon; return <div className="space-y-6"><header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm text-gray-500">Invoicing</p><h1 className="mt-1 text-2xl font-bold">{config.title}</h1><p className="mt-1 text-sm text-gray-500">{config.description}</p></div><button onClick={()=>setOpen(true)} className="rounded-xl bg-gray-900 px-4 py-2.5 text-sm font-semibold text-white dark:bg-white dark:text-gray-900"><Plus size={16} className="mr-2 inline"/>Add</button></header>{error&&<div className="rounded-xl bg-red-50 p-4 text-sm text-red-700">{error}</div>}<section className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">{loading?<div className="p-12 text-center"><Loader2 className="mx-auto animate-spin"/></div>:rows.length===0?<div className="p-12 text-center text-sm text-gray-500"><Icon className="mx-auto mb-3"/>No records found.</div>:<div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-gray-50 dark:bg-gray-800"><tr>{config.fields.map(f=><th key={f.key} className="px-5 py-3">{f.label}</th>)}</tr></thead><tbody className="divide-y dark:divide-gray-800">{rows.map((row,i)=><tr key={row.id||i}>{config.fields.map(f=><td key={f.key} className="px-5 py-4">{String(row[f.key]??"—")}</td>)}</tr>)}</tbody></table></div>}</section>{open&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="w-full max-w-lg rounded-2xl bg-white p-6 dark:bg-gray-900"><div className="mb-5 flex items-center justify-between"><h2 className="font-bold">Add {config.title}</h2><button onClick={()=>setOpen(false)}><X/></button></div><div className="space-y-4">{config.fields.map(f=><div key={f.key}><label className="mb-1 block text-xs font-medium">{f.label}</label><input type={f.type||"text"} value={form[f.key]||""} onChange={e=>setForm(v=>({...v,[f.key]:e.target.value}))} className="w-full rounded-xl border px-3 py-2.5 dark:border-gray-700 dark:bg-gray-800"/></div>)}</div><div className="mt-6 flex justify-end gap-2"><button onClick={()=>setOpen(false)} className="rounded-xl border px-4 py-2.5">Cancel</button><button disabled={saving} onClick={save} className="rounded-xl bg-gray-900 px-4 py-2.5 font-semibold text-white">{saving?"Saving…":"Save"}</button></div></div></div>}</div>;
}
const resource=(title:string,description:string,endpoint:string,icon:LucideIcon,fields:{key:string;label:string;type?:string}[])=> <ResourceManager config={{title,description,endpoint,icon,fields}}/>;
function CreditNotesManager(){return resource("Credit Notes","Manage invoice credits and adjustments.","/api/credit-notes",FileMinus2,[{key:"credit_note_number",label:"Credit note"},{key:"invoice_number",label:"Invoice"},{key:"amount",label:"Amount"},{key:"status",label:"Status"},{key:"issue_date",label:"Date"}]);}
function RecurringInvoicesManager(){return resource("Recurring Invoices","Manage recurring invoice schedules.","/api/recurring-invoices",Repeat,[{key:"name",label:"Name"},{key:"customer_name",label:"Customer"},{key:"frequency",label:"Frequency"},{key:"status",label:"Status"},{key:"next_invoice_date",label:"Next invoice"}]);}
function InvoiceRemindersManager(){return resource("Invoice Reminders","Manage payment reminder rules.","/api/invoice-reminders",Bell,[{key:"name",label:"Name"},{key:"days_after_due",label:"Days after due",type:"number"},{key:"channel",label:"Channel"},{key:"is_active",label:"Active"}]);}
function InvoiceTemplatesManager(){return resource("Invoice Templates","Manage invoice document templates.","/api/invoice-templates",FileText,[{key:"name",label:"Name"},{key:"description",label:"Description"},{key:"is_default",label:"Default"},{key:"is_active",label:"Active"}]);}
function TaxRatesManager(){return resource("Tax Rates","Manage tax rates used by invoice items.","/api/tax-rates",Percent,[{key:"name",label:"Name"},{key:"rate",label:"Rate",type:"number"},{key:"description",label:"Description"},{key:"is_active",label:"Active"}]);}
function InvoiceSettingsManager(){
 const sections=["General","Numbering","Company & Branding","Taxes","Payment","Templates","Email","WhatsApp","Reminders","Sharing","Automation","Approval"];
 const [active,setActive]=useState("General"),[settings,setSettings]=useState<any>(null),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[error,setError]=useState(""),[success,setSuccess]=useState("");
 const load=useCallback(async()=>{try{setLoading(true);const r=await fetch("/api/invoice-settings",{credentials:"include"}),d=await r.json();if(!r.ok)throw new Error(d.error||"Failed to load settings.");setSettings(d);}catch(e){setError(e instanceof Error?e.message:"Failed to load settings.");}finally{setLoading(false);}},[]);
 useEffect(()=>{load();},[load]);
 const save=async()=>{try{setSaving(true);const r=await fetch("/api/invoice-settings",{method:"PUT",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(settings)}),d=await r.json();if(!r.ok)throw new Error(d.error||"Failed to save settings.");setSettings(d);setSuccess("Settings saved successfully.");setTimeout(()=>setSuccess(""),2500);}catch(e){setError(e instanceof Error?e.message:"Failed to save settings.");}finally{setSaving(false);}};
 if(loading)return <div className="p-12 text-center"><Loader2 className="mx-auto animate-spin"/></div>;
 if(!settings)return <div className="p-12 text-center text-gray-500">No invoice settings found.</div>;
 const groups:Record<string,{key:string;label:string;type?:string}[]>={
  General:[{key:"default_currency",label:"Default Currency"},{key:"default_due_days",label:"Default Due Days",type:"number"}],
  Numbering:[{key:"invoice_prefix",label:"Invoice Prefix"},{key:"invoice_next_number",label:"Next Number",type:"number"},{key:"invoice_number_padding",label:"Number Padding",type:"number"}],
  "Company & Branding":[{key:"company_name",label:"Company Name"},{key:"company_email",label:"Email"},{key:"company_phone",label:"Phone"},{key:"company_tax_id",label:"Tax ID / VAT"},{key:"company_address",label:"Address"},{key:"company_website",label:"Website"}],
  Taxes:[{key:"default_tax_rate",label:"Default Tax Rate",type:"number"}],
  Payment:[{key:"payment_terms",label:"Payment Terms"},{key:"payment_methods",label:"Payment Methods"}],
  Templates:[{key:"default_template_id",label:"Default Template"}],
  Email:[{key:"email_subject",label:"Email Subject"},{key:"email_message",label:"Email Message"}],
  WhatsApp:[{key:"whatsapp_message",label:"WhatsApp Message"}],
  Reminders:[{key:"reminder_enabled",label:"Enable Reminders"},{key:"reminder_days_before",label:"Days Before Due",type:"number"},{key:"reminder_days_after",label:"Days After Due",type:"number"}],
  Sharing:[{key:"public_invoice_links",label:"Public Invoice Links"}],
  Automation:[{key:"auto_send_invoices",label:"Auto Send Invoices"}],
  Approval:[{key:"require_invoice_approval",label:"Require Invoice Approval"}],
 };
 const fields=groups[active]||[];
 const booleanField=(key:string)=>/enabled|links|auto_send|approval/i.test(key);
 return <div className="space-y-6"><header><p className="text-sm text-gray-500">Invoicing · Settings</p><h1 className="mt-1 text-2xl font-bold">Invoice Settings</h1><p className="mt-1 text-sm text-gray-500">Configure how SaMi handles your invoices.</p></header>{error&&<div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div>}{success&&<div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700">{success}</div>}<div className="grid gap-6 lg:grid-cols-[280px_1fr]"><aside className="rounded-2xl border border-gray-200 bg-white p-2 dark:border-gray-800 dark:bg-gray-900">{sections.map(section=><button key={section} type="button" onClick={()=>setActive(section)} className={`w-full rounded-xl px-3 py-2.5 text-left text-sm font-semibold ${active===section?"bg-blue-600 text-white":"text-gray-600 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-800"}`}>{section}</button>)}</aside><section className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900"><h2 className="text-lg font-bold">{active}</h2><p className="mt-1 text-sm text-gray-500">Configure {active.toLowerCase()} settings.</p><div className="mt-6 grid gap-4 md:grid-cols-2">{fields.map(field=><div key={field.key}><label className="mb-1 block text-xs font-medium">{field.label}</label>{booleanField(field.key)?<select value={settings[field.key]?"true":"false"} onChange={e=>setSettings((v:any)=>({...v,[field.key]:e.target.value==="true"}))} className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800"><option value="true">Enabled</option><option value="false">Disabled</option></select>:<input type={field.type||"text"} value={settings[field.key]??""} onChange={e=>setSettings((v:any)=>({...v,[field.key]:field.type==="number"?Number(e.target.value):e.target.value}))} className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800"/>}</div>)}</div><div className="mt-6 flex justify-end"><button onClick={save} disabled={saving} className="rounded-xl bg-gray-900 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50 dark:bg-white dark:text-gray-900">{saving?"Saving…":"Save Settings"}</button></div></section></div></div>;
}
