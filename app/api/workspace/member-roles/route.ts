...
function handleError(error: unknown) {
  ...
  if (error instanceof UserRoleServiceError) {
    switch (error.code) {
      ...
      case 'EMPTY_ROLE_SET':
        return json({ success: false, code: error.code, error: error.message }, 409);
  ...
  console.error('[SaMi] Member role request failed:', error);
  return json({
    success: false,
    code: 'MEMBER_ROLES_FAILED',
    error: 'SaMi could not complete the role assignment request.',
  }, 500);
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!userId) {
      return json({ success: false, code: 'USER_ID_REQUIRED', error: 'User ID is required.' }, 400);
    }

    const [member, roles] = await Promise.all([
      getWorkspaceMemberRoles(userId),
      listAssignableWorkspaceRoles(),
    ]);

    const roleAppAccess = await getRoleAppAccessMap(roles.map(role => role.id));
    const byRole = new Map(roleAppAccess.map(item => [item.roleId, item]));

    return json({
      success: true,
      member,
      roles: roles.map(role => {
        const access = byRole.get(role.id);
        return {
          ...role,
          apps: access?.apps || [],
