...
import { Sun, Moon, ArrowRight, Building2, Phone, Mail, User, Loader2, X, AlertTriangle } from 'lucide-react';
...
function GoogleCompleteContent() {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      sessionStorage.setItem('sami_account_form', JSON.stringify(accountData));
      sessionStorage.setItem('sami_google_auth', 'true');

      router.push('/auth/select-apps');
    } catch {
      setOverlay({ type: 'error', title: 'Error', message: 'Something went wrong.' });
      setLoading(false);
    }
  };
        ...
        <section className="bg-white dark:bg-[#111418] border border-gray-200 dark:border-gray-800 rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.06)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.25)] overflow-hidden">
          <div className="px-8 py-8 sm:px-10 sm:py-9">
            ...
            <div className="mb-7">
              ...
              <p className="mt-2 text-[14px] text-gray-500 dark:text-gray-400">Just one more step. Enter your business name.</p>
            </div>

            {/* Google Account Info */}
            <div className="mb-7 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl flex items-center gap-4">
              {form.avatarUrl && (
                <img src={form.avatarUrl} alt="Google avatar" className="h-10 w-10 rounded-full" />
              )}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{form.firstName} {form.lastName}</p>
            ...
            <div className="space-y-5">
              ...
              <div className="pt-6 border-t border-gray-100 dark:border-gray-800 flex items-center justify-end">
                <button
                  onClick={handleNext}
                  disabled={loading}
                  className="min-w-[145px] h-[44px] px-5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-[13px] font-semibold flex items-center justify-center gap-2 transition disabled:opacity-60"
                >
                  {loading ? <Loader2 size={16} className="animate-spin" /> : <>Next: Select Apps <ArrowRight size={16} /></>}
                </button>
              </div>
            </div>
          </div>
