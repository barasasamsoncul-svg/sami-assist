      ...
      <div className="text-center max-w-md">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 shadow-xl border border-gray-200 dark:border-gray-800">
          ...
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            We sent a verification link to <strong className="text-gray-900 dark:text-white">{email || 'your email'}</strong>.
          </p>

          {message && <div className="mt-4 p-3 bg-green-50 dark:bg-green-900/20 rounded-xl text-green-700 dark:text-green-400 text-sm">{message}</div>}
          {error && <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/20 rounded-xl text-red-700 dark:text-red-400 text-sm">{error}</div>}
          ...
          <Link href="/auth/login" className="mt-4 inline-block w-full px-4 py-3 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition">
            Go to Login
          </Link>
        </div>
      </div>
