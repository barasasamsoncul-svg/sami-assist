...
function hasPeopleVisibility(context: Awaited<ReturnType<typeof getPermissionContext>>): boolean {
  return (
...
function hasCompanyVisibility(context: Awaited<ReturnType<typeof getPermissionContext>>): boolean {
  return (
    permissionContextHas(context, SAMI_PERMISSIONS.COMPANIES_VIEW) ||
    permissionContextHas(context, SAMI_PERMISSIONS.COMPANIES_MANAGE)
  );
}

async function requireMemberCompanyView() {
  const context = await getPermissionContext();

  if (context.isOwner || (hasPeopleVisibility(context) && hasCompanyVisibility(context))) {
    return context;
  }

  if (!hasPeopleVisibility(context)) {
    throw new MemberCompanyAccessError(
      'USERS_VIEW_REQUIRED',
      'You need People visibility before you can inspect employee company access.',
    );
  }

  throw new MemberCompanyAccessError(
    'COMPANIES_VIEW_REQUIRED',
    'You do not have permission to view company access.',
  );
}

async function requireMemberCompanyManage() {
  const context = await requireMemberCompanyView();

  if (context.isOwner || permissionContextHas(context, SAMI_PERMISSIONS.COMPANIES_MANAGE)) {
    return context;
  }

  throw new MemberCompanyAccessError(
    'COMPANIES_MANAGE_REQUIRED',
    'You do not have permission to assign company scope to employees.',
  );
}
...
export async function getMemberCompanyAccess(userIdInput: string): Promise<MemberCompanyAccessState> {
  ...
  return {
    ...
    userId,
    editable: target.is_owner !== true && userId !== context.userId && (context.isOwner || permissionContextHas(context, SAMI_PERMISSIONS.COMPANIES_MANAGE)),
    defaultCompanyId,
    companyIds,
...
export async function replaceMemberCompanyAccess(input: {
  ...
  const context = await requireMemberCompanyManage();
