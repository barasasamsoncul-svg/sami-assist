...
async function requireAnyPermission(
  ...
  deniedCode: AppAccessErrorCode,
  message: string,
): Promise<PermissionContext> {
  const context = await getPermissionContext();
  if (context.isOwner || permissions.some(permission => permissionContextHas(context, permission))) {
    return context;
  }
  throw new AppAccessError(deniedCode, message);
}

async function requireUsersView(): Promise<PermissionContext> {
  return requireAnyPermission(
    [SAMI_PERMISSIONS.USERS_VIEW, SAMI_PERMISSIONS.USERS_MANAGE],
    'USERS_VIEW_REQUIRED',
    'You do not have permission to view employee app access.',
  );
}

async function requireUsersManage(): Promise<PermissionContext> {
  return requireAnyPermission(
    [SAMI_PERMISSIONS.USERS_MANAGE],
    'USERS_MANAGE_REQUIRED',
    'You do not have permission to manage employee app access.',
  );
}

async function requireInvitationsView(): Promise<PermissionContext> {
  return requireAnyPermission(
    [SAMI_PERMISSIONS.INVITATIONS_VIEW, SAMI_PERMISSIONS.INVITATIONS_MANAGE],
    'INVITATIONS_VIEW_REQUIRED',
    'You do not have permission to view invitation app access.',
  );
}

async function requireInvitationsManage(): Promise<PermissionContext> {
  return requireAnyPermission(
    [SAMI_PERMISSIONS.INVITATIONS_MANAGE],
    'INVITATIONS_MANAGE_REQUIRED',
    'You do not have permission to manage invitation app access.',
  );
}
...
export async function listInvitationAppOptions(): Promise<WorkspaceAppOption[]> {
  const context = await requireInvitationsManage();
...
export async function getInvitationAppAccesses(): Promise<InvitationAppAccessState[]> {
  const context = await requireInvitationsView();
  const result = await queryControl(`
    SELECT
